...
Trained equation (variables: θ, k, r=resolution, z):
```
((-4.6036716 - exp(4.438199 * ((r - z) - square(k)))) * r) + ((2.2928627 - z) + θ)
```

At HF (r = 0.8):
```
θ - z - 27.866232431119*exp(-4.438199*z - 4.438199*square(k)) - 1.39007458
```
At LF (r = 0.4):
```
θ - z - 2.36077243423076*exp(-4.438199*z - 4.438199*square(k)) + 0.45139406
```
Resolution correction in flux_norm space (HF − LF, simplified):
```
-25.5054599968883*exp(-4.438199*z - 4.438199*square(k)) - 1.84146864
```
...
Trained equation (variables: θ, k, r=resolution, z):
```
((square(r * ((0.34314474 - z) * r)) * -148.04918) + square((k * exp(θ)) - z)) - r
```

At HF (r = 0.8):
```
-148.04918*square(0.2196126336 - 0.64*z) + square(k*exp(θ) - z) - 0.8
```
At LF (r = 0.4):
```
-148.04918*square(0.0549031584 - 0.16*z) + square(k*exp(θ) - z) - 0.4
```
Resolution correction in flux_norm space (HF − LF, simplified):
```
148.04918*square(0.0549031584 - 0.16*z) - 148.04918*square(0.2196126336 - 0.64*z) - 0.4
```
