...
def test_derivative_faithful_multiz_aggregates_over_z():
    ...
    z_grid = np.array([3.4, 3.6], dtype=float)
    fid = _make_fid()

    # GP slope = 1 at z=3.4, but trick via a per-z GP:
    class _PerZGP:
        def predict(self, theta, k, z):
            k = np.asarray(k, dtype=float)
            # slope 1.0 regardless of z — gradient always finite
            return 1.0 * float(np.asarray(theta)[2]) * np.ones_like(k)

    # Equation slope = 1 exactly -> faithful (median 0.0 <= 0.25)
    refit = _StubRefit(slope=1.0)
    result = derivative_faithful_multiz(
        refit=refit, gp=_PerZGP(), fid=fid, fid_value=fid[2],
        k_grid=k_grid, z_grid=z_grid, param_idx=2, tol=0.25,
    )
    assert result is True
